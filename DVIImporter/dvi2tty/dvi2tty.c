...
char *str;
{
    ...
    DVIfilename = malloc( sizeof(char) * (i+ 1) );
    int newlen = strlcpy(DVIfilename, str, i+1);
    assert(newlen == i);
    
    if (!((i >= 5) && (str[i-1] == 'i') && (str[i-2] == 'v') &&
          (str[i-3] == 'd') && (str[i-4] == '.'))) {
        DVIfilename = realloc(DVIfilename, sizeof(char) * 5);
        strlcat(DVIfilename, ".dvi", strlen(DVIfilename)-5);
    }
