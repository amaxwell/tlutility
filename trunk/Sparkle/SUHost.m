...
- (id)initWithBundle:(NSBundle *)aBundle
{
	...
	if ((self = [super init]))
	{
		...
		if (![bundle bundleIdentifier])
			NSLog(@"Sparkle Error: the bundle being updated at %@ has no CFBundleIdentifier! This will cause preference read/write to not work properly.", [bundle bundlePath]);
    }
    return self;
}
...
+ (NSString *)systemVersionString
{
	...
	if (!err1 && !err2 && !err3)
	{
		verStr = [NSString stringWithFormat:@"%d.%d.%d", (int32_t)major, (int32_t)minor, (int32_t)bugfix];
	}
	else
#endif
	{
